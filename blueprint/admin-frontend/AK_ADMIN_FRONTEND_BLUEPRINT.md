...
系统设置包含独立“第三方登录配置”页面：首批能力固定注册微信、GitHub、Apple 与 Google，通过 capability catalog 与编译期强类型表单双重约束；Secret 仅写入服务端，预检通过后才能启用。应用管理的客户端配置按权限显示第三方登录 Tab，并通过一个原子请求保存四个平台绑定。平台可在后续代码注册表、OpenAPI、数据库迁移和双语资源同步扩展，禁止把在线字典变成可执行 OAuth 插件。

...
```bash
sh install.sh --version 0.5.0-preview.3 --install-dir "$HOME/.local/bin"
sh install.sh --version 0.5.0-preview.3 --prefix "$HOME/.local"
```
...
```bash
make release VERSION=0.5.0-preview.3
npm run release -- --version 0.5.0-preview.3
```

确认所有输出后，显式启用发布：

```bash
make release VERSION=0.5.0-preview.3 PUBLISH=1
npm run release -- --version 0.5.0-preview.3 --publish
```
...
当前流程只允许带 prerelease 后缀的 Preview，例如 `v0.5.0-preview.3`。无后缀稳定 tag 会同时被本地编排器、主发布 workflow 和可人工重跑的渠道 workflow 拒绝；两个 workflow 还会复核签名 tag、检出 commit 与 `origin/main` 的可达关系。

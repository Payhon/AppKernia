...
## 2026-08-05 Mobile 四页面 ImageGen 视觉概念
...
- 文章页面是堆栈视觉概念，不修改既有三 Tab。当前 Mobile 机器契约没有 Article route/API/permission，本轮不伪造接口或实现状态。
...
- 本轮只完成 raster 视觉审查和静态蓝图/i18n 校验；没有 UVue 组件、API、Route Registry、语言包或业务实现变更。
...
- `000012_region_management` 增加 `version/is_manually_managed/deleted_at` 与活动父级索引；所有地区读取和 `has_children` 排除软删除。核心目录 upsert 对手工记录保持现值，因此不会覆盖编辑或恢复已删除节点。
...
- 真实检查 [CI run 31014624161](https://github.com/Payhon/AppKernia/actions/runs/31014624161)：`admin` 失败点为 `actions/setup-node@v5` 在 pnpm 尚未安装时初始化 pnpm cache；`mobile-blueprint` 失败点为 `check-project.sh` 调用 runner 中不存在的 `rg`。
- Admin job 新增 `pnpm/action-setup@v4` 并锁定 `11.18.0`，放在 `actions/setup-node@v5` 之前；移除后置且无法修复 setup-node cache 阶段的 Corepack 两步。
- Mobile job 在静态项目检查前通过 Ubuntu apt 显式安装 `ripgrep`，不改动 Mobile 扫描规则和业务源码。

### 真实命令与退出码

| 命令 / 阶段 | Exit | 真实结果 |
|---|---:|---|
| `gh auth status` / `gh run view 31014624161 --log-failed` | 0 / 0 | 确认两个失败 job 的原始日志与退出阶段；backend、blueprint job 原 run 已成功 |
| `apps/ak-mobile/scripts/check-project.sh` | 0 | Mobile Blueprint、i18n、VDOM、禁用模式扫描和三平台脚本目标检查通过；平台构建仍是独立证据门禁 |
| `python3 blueprint/mobile/scripts/validate_blueprint_specs.py` / `python3 blueprint/scripts/validate_i18n_contract.py` | 0 / 0 | Mobile 0 error/warning；`zh-CN`/`en-US` 契约通过 |
| `pnpm install --frozen-lockfile` / `pnpm --filter @appkernia/admin check` | 0 / 0 | lint、strict typecheck、20 files / 75 tests、production build、bundle budget、Admin Blueprint 均通过 |
| Ruby YAML parse / CI prerequisite order assertions / `actionlint v1.7.7` | 0 / 0 / 0 | workflow 语法、四个 job、pnpm 安装顺序和 ripgrep 准备步骤通过静态校验 |
| `git diff --check -- .github/workflows/ci.yml` | 0 | workflow 补丁格式通过 |

### 验证边界与风险

- 本机 Node 为 26.5.0，Admin 命令提示仓库 Node 24 engine warning；CI 明确固定 Node 24.18.1，pnpm 固定 11.18.0。
- 修复尚未 commit/push，GitHub Actions 不能对未发布 workflow 重跑，因此远端绿色状态仍待提交后的新 run 验证。
- 本轮没有修改 Backend、Admin/Mobile 业务代码、OpenAPI、数据库、权限或可见 UI；未执行 Android/iOS/Harmony 构建或真机测试。

...
- `iam/security/interactive_captcha.type` uses the fixed `click | slide | drag | rotate` enum shared by Admin login and Mobile SMS protection. It is read-only unless the current account is a `super-admin` in the configured platform tenant and has `sys.platform_config.update`.

...
- 头像文件选择后先进入独立裁剪 Modal；裁剪视口为正方形并以圆形安全区预览最终头像，确认后才生成 512×512 上传文件。
...
- 裁剪支持拖动、缩放、90 度旋转；必须同时提供键盘可操作的方向微调、缩放 Slider 和具名旋转按钮，拖动不是唯一交互。
- 本地预览 object URL 在替换或卸载时释放；上传能力、对象存储签名和文件二进制不持久化。
- 裁剪确认后展示独立预览和“重新裁剪/上传”动作；上传成功同步 Profile Query、Auth Context 与私有头像 Blob，不刷新页面。

...
- Place `admin.login_captcha.type` in the localized `iam / security` category. The protocol values are the fixed enum `click | slide | drag | rotate`; labels are translated without changing the submitted value.

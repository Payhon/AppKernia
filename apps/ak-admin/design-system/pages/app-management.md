...
- The client configuration Modal is 760 px on desktop and full-screen at narrow widths. Tabs remain in code-defined order: Sharing, Scanner, then Third-party sign-in; each Tab is omitted unless its own read permission is present, and switching Tabs never saves or discards local state.
...
- The Third-party sign-in Tab renders WeChat, GitHub, Apple and Google as stacked cards in stable provider order. Each card pairs its platform/build-variant labels with a configuration selector and enable switch. Only active, preflight-ready tenant configurations are selectable; invalid existing references remain visible as unavailable and fail closed.
- Submit all four provider bindings through one atomic action. A 409 retains local state and explicitly states that nothing partially applied. Disabling preserves the selected configuration for intentional re-enable, while clearing a configuration forces disabled on save.

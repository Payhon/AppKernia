export const clientConfigTabPermissions = [
  ...
  { id: "login-providers", readPermission: "app.login_settings.read", updatePermission: "app.login_settings.update" },
] as const;
...
export function canReadAnyClientConfig(permissions: ReadonlySet<string>): boolean {
  return clientConfigTabPermissions.some((tab) => permissions.has(tab.readPermission)) || permissions.has("app.login_provider_binding.read");
}

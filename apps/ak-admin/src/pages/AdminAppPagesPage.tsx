...
function PageDrawer({ activeLocale, editor, form, fullScreen, languages, saving, onActiveLocaleChange, onClose, onSave }: { activeLocale: AdminLocale; editor: Editor; form: ReturnType<typeof useForm<AppPageEditorInput>>; fullScreen: boolean; languages: SystemLanguagesState; saving: boolean; onActiveLocaleChange: (locale: AdminLocale) => void; onClose: () => void; onSave: () => void }) {
  ...
  return <Drawer destroyOnHidden extra={<Button disabled={!languages.isReady} loading={saving} type="primary" onClick={onSave}>{t("common.actions.save")}</Button>} onClose={onClose} open={editor !== null} size={fullScreen ? "100%" : "large"} title={t(editor === "new" ? "apps.pages.editor.create" : "apps.pages.editor.edit")}><Form layout="vertical"><Form.Item label={t("apps.pages.fields.slug")} {...validation(errors.slug?.message)}><Controller control={form.control} name="slug" render={({ field }) => <Input {...field} aria-label={t("apps.pages.fields.slug")} disabled={protectedType} />} /></Form.Item><Form.Item label={t("apps.pages.fields.type")} {...validation(errors.page_type?.message)}><Controller control={form.control} name="page_type" render={({ field }) => <Select {...field} aria-label={t("apps.pages.fields.type")} disabled={protectedType} options={["privacy-policy", "terms-of-service", "about-us", "custom"].map((value) => ({ value, label: t(`apps.pages.type.${value}`) }))} />} /></Form.Item>{protectedType ? <Alert showIcon type="info" title={t("apps.pages.reserved_hint")} /> : null}<AkLocalizedFormTabs activeLocale={activeLocale} errorLocales={{ "en-US": Boolean(errors.translations?.["en-US"]), "zh-CN": Boolean(errors.translations?.["zh-CN"]) }} languages={languages} onActiveLocaleChange={onActiveLocaleChange} renderFields={(locale, label) => <><Form.Item label={t("apps.pages.fields.title")} {...validation(errors.translations?.[locale]?.title?.message)}><Controller control={form.control} name={`translations.${locale}.title`} render={({ field }) => <Input {...field} aria-label={`${label} ${t("apps.pages.fields.title")}`} />} /></Form.Item><Form.Item label={t("apps.pages.fields.body_format")} {...validation(errors.translations?.[locale]?.body_format?.message)}><Controller control={form.control} name={`translations.${locale}.body_format`} render={({ field }) => <Select {...field} aria-label={`${label} ${t("apps.pages.fields.body_format")}`} options={["markdown", "blocks"].map((value) => ({ value, label: t(`apps.pages.body_format.${value}`) }))} />} /></Form.Item><Form.Item label={t(`apps.pages.fields.${form.watch(`translations.${locale}.body_format`) === "blocks" ? "blocks" : "body"}`)} extra={form.watch(`translations.${locale}.body_format`) === "blocks" ? t("apps.pages.fields.blocks_hint") : undefined} {...validation(errors.translations?.[locale]?.body?.message)}><Controller control={form.control} name={`translations.${locale}.body`} render={({ field }) => <Input.TextArea {...field} aria-label={`${label} ${t("apps.pages.fields.body")}`} maxLength={100000} rows={16} showCount />} /></Form.Item></>} /></Form></Drawer>;
}

...
def main()->None:
    ...
    with sync_playwright() as p:
        ...
        with page.expect_response(lambda r:"/admin-api/v1/regions?" in r.url and "parent_code=" in r.url) as child_response: page.get_by_role("button",name="Expand region",exact=True).first.click()
        assert child_response.value.status==200;page.get_by_text("E2E Child",exact=True).wait_for();evidence["lazy_child_status"]=child_response.value.status

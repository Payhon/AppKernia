...
def main():
 ...
 with sync_playwright() as p:
  ...
  assert revoked.value.status==200;page.get_by_text("访问控制规则已撤销",exact=True).wait_for();page.get_by_label("显示语言",exact=True).select_option(label="English");open_system_page(page,"/system/monitoring/health","Monitoring");page.wait_for_url(f"{BASE}/system/monitoring/health");ensure_english(page);page.get_by_role("heading",name="Service Status",exact=True).wait_for();body=page.locator("body").inner_text().lower();assert "postgres://" not in body and "appkernia-dev-only" not in body and "/users/" not in body;shot(page,"health.en-US.1440",evidence);page.get_by_label("Display language",exact=True).select_option(label="简体中文")

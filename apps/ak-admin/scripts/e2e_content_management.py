...
def handler(route:Route):
 ...
 if u.endswith("/dictionaries/system.language"):
  ...
  return respond(route,200,{"code":"system.language","locale":"en-US" if english else "zh-CN","extension_policy":"fixed","items":[{"value":"zh-CN","label":"Simplified Chinese" if english else "简体中文","is_default":True,"extra":{}},{"value":"en-US","label":"English","is_default":False,"extra":{}}]})
...
def main():
 ...
 with sync_playwright() as p:
  ...
  print("content-e2e:article-drawer",flush=True); page.locator("button:has-text('编 辑')").click(); drawer=page.get_by_role("dialog"); drawer.wait_for(); assert drawer.get_by_role("tab",name="简体中文").get_attribute("aria-selected")=="true"; title=drawer.get_by_label("简体中文 标题"); title.fill("切换后保留的标题"); drawer.get_by_role("tab",name="English").click(); drawer.get_by_role("tab",name="简体中文").click(); assert title.input_value()=="切换后保留的标题"; page.screenshot(path=OUT/"drawer-zh-CN-1440.png",full_page=True); page.keyboard.press("Escape"); drawer.wait_for(state="hidden")
  print("content-e2e:categories-load",flush=True); page.get_by_role("tab",name="分类").click(); page.get_by_role("heading",name="分类管理").wait_for(); page.get_by_text("新闻").wait_for(); axe(page,"zh-CN.categories.1440",evidence); page.locator("button:has-text('编 辑')").click(); category_drawer=page.get_by_role("dialog"); category_drawer.wait_for(); assert category_drawer.get_by_role("tab",name="简体中文").count()==1; assert category_drawer.get_by_role("tab",name="English").count()==1; page.screenshot(path=OUT/"category-drawer-zh-CN-1440.png",full_page=True); page.keyboard.press("Escape"); category_drawer.wait_for(state="hidden")

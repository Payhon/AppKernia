...
WHERE tenant_id = $1 AND module_code = 'storage' AND config_group = 'cloud'
  AND status = 'active'`, tenantID)
	...
	profile := storageProfile{UploadPolicy: domain.UploadPolicy{
		...
		FileMediaTypes:  stringSlice(values, "storage.file_media_types", []string{"application/pdf", "application/json", "application/zip", "application/x-zip-compressed", "application/vnd.android.package-archive", "application/octet-stream", "text/plain", "text/csv", "image/jpeg", "image/png", "image/webp", "video/mp4"}),

...
func TestPublicReleaseResponseIsLocalizedAndDoesNotExposeAdminFields(t *testing.T) {
	value := publicReleaseResponse(profiledomain.Release{Platform: "ios", CurrentVersion: "2.0.0", MinimumVersion: "1.5.0", ReleaseNotes: map[string]string{"zh-CN": "中文说明", "en-US": "English notes"}, Active: true, LockVersion: 9}, i18n.LocaleEnUS)
	raw, err := json.Marshal(value)
	if err != nil {
		t.Fatal(err)
	}
	encoded := string(raw)
	if !strings.Contains(encoded, "English notes") || strings.Contains(encoded, "lock_version") || strings.Contains(encoded, "active") || strings.Contains(encoded, "\"id\"") {

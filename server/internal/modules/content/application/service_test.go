...
func (f *fakeRepository) ListPublished(_ context.Context, tenant, user uuid.UUID, locale string, _ content.PublicFilter) (content.PublicArticlePage, error) {
	if tenant == uuid.Nil || user == uuid.Nil || locale != "en-US" {
		return content.PublicArticlePage{}, errors.New("mobile scope missing")
	}
	return content.PublicArticlePage{}, nil
}
func (f *fakeRepository) ListPublishedCategories(_ context.Context, tenant uuid.UUID, locale string) (content.PublicCategoryPage, error) {
	if tenant == uuid.Nil || locale != "en-US" {
		return content.PublicCategoryPage{}, errors.New("category scope missing")
	}
	return content.PublicCategoryPage{}, nil
}
func (f *fakeRepository) OpenArticleAsset(_ context.Context, tenant, fileID uuid.UUID) (content.ArticleAsset, io.ReadCloser, error) {
	if tenant == uuid.Nil || fileID == uuid.Nil {
...
func auth(perms ...string) iam.AuthenticatedContext {
	return iam.AuthenticatedContext{AuthContext: iam.AuthContext{Tenant: iam.Tenant{ID: uuid.New()}, User: iam.User{ID: uuid.New()}, Permissions: perms}, SessionID: uuid.New()}
}
...
func TestCreateArticleScopesPrincipalAndRequiresBothLocales(t *testing.T) {
	repo := &fakeRepository{}
	service := NewService(fakeAuthenticator{value: auth("content.article.create"), want: adminAudience}, repo)
	if _, err := service.CreateArticle(context.Background(), "token", content.Principal{RequestID: "request"}, article()); err != nil {
	...
	if _, err := service.CreateArticle(context.Background(), "token", content.Principal{RequestID: "request"}, x); !errors.Is(err, content.ErrInvalid) {
...
func TestCreateArticleRejectsBodyFormatMismatchAndMissingPermission(t *testing.T) {
	repo := &fakeRepository{}
	service := NewService(fakeAuthenticator{value: auth("content.article.read"), want: adminAudience}, repo)
	if _, err := service.CreateArticle(context.Background(), "token", content.Principal{RequestID: "request"}, article()); !errors.Is(err, content.ErrForbidden) {
